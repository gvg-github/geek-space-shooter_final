...
public class Player extends Ship {
    ...
    public int getTecDmg() {
        return tecDmg;
    }

    public void setTecDmg() {
        if (this.score >= this.costDmg) {
            this.tecDmg++;
            this.score -= this.costDmg;
            this.costDmg *= this.tecDmg;
        }
    }
    ...
    public void renderHUD(SpriteBatch batch, BitmapFont fnt, BitmapFont fnt1, float x, float y) {
        ...
        batch.setColor(1, 1, 1, 1);
        hudStringHelper.setLength(0);
        hudStringHelper.append("Dmg: ").append(tecDmg);
        fnt.draw(batch, hudStringHelper, x + 4, y - 22);
        batch.setColor(1, 1, 1, 1);

//        batch.setBlendFunction(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA);
        if (score >= costDmg) {
            hudStringHelper.setLength(0);
            hudStringHelper.append("Dmg +1: ").append(costDmg);
            fnt1.draw(batch, hudStringHelper, x + 100, y - 22);
        }
